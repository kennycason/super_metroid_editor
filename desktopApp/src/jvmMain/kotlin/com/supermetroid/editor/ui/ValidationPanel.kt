...
fun ValidationPanel(
    ...
    val parser = romParser ?: run {
        Box(modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("Load a ROM to run validation", color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        return
    }

    var issues by remember { mutableStateOf<List<RomValidator.Issue>?>(null) }
    var scanTimeMs by remember { mutableStateOf(0L) }
    ...
    Column(modifier = modifier.fillMaxSize().padding(12.dp)) {
        Row(
            ...
            Button(onClick = {
                ...
                issues = RomValidator.validate(parser, roomIds)
                scanTimeMs = System.currentTimeMillis() - start
            }) {
                Text("Run Validation", fontSize = 12.sp)
            }
            if (issues != null) {
        ...
        val results = issues
        if (results == null) {
            Text("Click 'Run Validation' to scan the ROM for issues.",
            ...
            Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState())) {
                for ((category, categoryIssues) in grouped) {
                    ...
                    val catWarnings = categoryIssues.count { it.severity == RomValidator.Severity.WARNING }
                    Text(
