...
object SmEditRoomStatePredicateFormat {
    ...
    val routineBytes: ByteArray = RoutineBuilder().run {
        ...
        emit(0x53, 0x4D, 0x45, 0x44, 0x50, 0x52, 0x45, 0x44, 0x01) // SMEDPRED, v1
        build()
    }

    fun matchesAt(bytes: ByteArray, pcOffset: Int): Boolean =
        pcOffset >= 0 && pcOffset + routineBytes.size <= bytes.size &&
            routineBytes.indices.all { bytes[pcOffset + it] == routineBytes[it] }
}
