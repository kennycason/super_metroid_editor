...
class ProjectRoomStatesTest {
    ...
    fun `typed equipment capacity pickup and global boss predicates survive ROM round trip`() {
        ...
        val inspection = exported.inspectRoomStates(roomId)
        assertTrue(inspection.isComplete)
        val typed = inspection.states.drop(inspection.states.size - conditions.size - 1).dropLast(1).map { it.condition }
        assertEquals(
            listOf(
        ...
        assertEquals(
            listOf(0x0001, 25, 0x51, packedBossConditionArgument(4, 0x02), packedBossConditionArgument(7, 0x80)),
            typed.map { it.argument },
        )
        assertEquals(listOf(false, false, false, true, false), typed.map { it.negated })
        ...
        val reopenedTyped = reopened.drop(reopened.size - conditions.size - 1).dropLast(1).map { it.condition }
