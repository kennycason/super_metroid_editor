...
| Impulse Tracker | `.it` | Import | Tracker module import path used by mITroid-style workflows | The importer parses IT headers, sample metadata, orders, packed patterns, speed-adjusted note timing, tempo commands, note stops, and first-8-channel note data. For sample-mode modules with embedded mono uncompressed PCM samples, it converts samples to BRR and stores a native payload containing N-SPC sequence data, instrument table bytes, sample directory entries, and BRR sample blocks. IT-compressed samples, stereo samples, instrument-mode sample maps/envelopes, and most tracker effects are reported and fall back to editable sequence import without a custom BRR payload. |
